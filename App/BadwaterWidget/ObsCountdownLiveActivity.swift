    ...
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: ObsActivityAttributes.self) { context in
            ...
            LockScreenCountdown(context: context)
                .padding()
                .activityBackgroundTint(BadwaterColor.surface)
                .activitySystemActionForegroundColor(BadwaterColor.ink)
    ...
    var body: some View {
        ...
        HStack(spacing: 6) {
            Text(overdue ? "Obs overdue" : "Next obs in")
                .font(.caption)
                .foregroundStyle(overdue ? BadwaterColor.caution : BadwaterColor.muted)
            Text(timerInterval: countdownRange(to: dueAt), countsDown: !overdue)
                .monospacedDigit()
                .foregroundStyle(overdue ? BadwaterColor.caution : BadwaterColor.ink)
        }
    }
}
