...
struct ShiftTrendChart: View {
                    ...
                    ForEach(day.points) { p in
                        AreaMark(x: .value("Time", p.clock), y: .value("Value", p.value))
                            .foregroundStyle(LinearGradient(
                                colors: [PlateworksColor.accent.opacity(0.20), PlateworksColor.accent.opacity(0.02)],
    ...
    private var styleRange: [Color] {
        ...
        return days.enumerated().map { i, d in
            d.isLatest
                ? PlateworksColor.accent
                : PlateworksColor.muted.opacity(0.30 + 0.35 * (Double(i) / Double(n)))
        }
    }
}
