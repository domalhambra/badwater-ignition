...
// (conformance/vectors.json). BadwaterCore is the source of truth: everything
...
//
// Usage:
//   swift run badwater-vectors                 # print to stdout
//   swift run badwater-vectors --out FILE      # write FILE
//   swift run badwater-vectors --check FILE    # exit 1 if FILE is stale
...
"meta": {
 "version": 1,
 "source": "Sources/BadwaterCore (IRPG PMS 461 / PMS 437)",
 "generatedBy": "swift run badwater-vectors --out conformance/vectors.json",
...
if let i = args.firstIndex(of: "--out"), i + 1 < args.count {
    ...
    guard let existing = try? String(contentsOfFile: path, encoding: .utf8) else {
        fail("STALE VECTORS: \(path) is missing. Run: swift run badwater-vectors --out \(path)")
    }
    if existing == output {
        ...
        fail("""
        STALE VECTORS: \(path) no longer matches BadwaterCore (first diff at line \(line + 1)).
          committed: \(have)
          expected:  \(want)
        The core changed without regenerating the conformance vectors.
        Run: swift run badwater-vectors --out \(path)  — then bring web/engine.js to parity
